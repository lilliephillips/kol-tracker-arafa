...
// ─── TIKTOK ───────────────────────────────────────────────────
// Pakai: scraperx/tiktok-data-extractor-scraper
// Support video URL langsung + auto proxy fallback
async function scrapeTikTok(url) {
  const items = await runAndWait('scraperx~tiktok-data-extractor-scraper', {
    videoUrls: [url],
    resultsPerPage: 1,
  ...
  return {
    ...
    likes:    post.diggCount    ?? post.likeCount           ?? post.stats?.diggCount  ?? 0,
...
// ─── INSTAGRAM ────────────────────────────────────────────────
// Pakai: apify/instagram-post-scraper
...
// ─── YOUTUBE ──────────────────────────────────────────────────
// Pakai: streamers/youtube-scraper
